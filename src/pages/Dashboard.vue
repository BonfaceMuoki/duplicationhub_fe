    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-md p-6">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-semibold text-gray-900 flex items-center gap-2">
          <i class="fas fa-user-shield text-indigo-500"></i> User Profile & Permissions
        </h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="font-medium text-gray-900">{{ userInfo?.name || 'Loading...' }}</p>
              </div>
            </div>
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-gradient-to-br from-green-100 to-blue-100 rounded-full flex items-center justify-center">
                <i class="fas fa-envelope text-green-600"></i>
              </div>
              <div>
                <p class="text-sm text-gray-500">Email</p>
                <p class="font-medium text-gray-900">{{ userInfo?.email || 'Loading...' }}</p>
              </div>
            </div>
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="font-medium text-gray-900">{{ userRole || 'Loading...' }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Permissions -->
        <div class="space-y-4">
          <h3 class="text-lg font-medium text-gray-900 border-b border-gray-200 pb-2">Permissions</h3>
          
          <!-- Debug Info (remove this in production) -->
          <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg mb-4">
            <p class="text-xs text-yellow-800">
              <strong>Debug:</strong> Permissions data type: {{ typeof userPermissions }}, 
              Keys: {{ userPermissions ? Object.keys(userPermissions) : 'null' }}
            </p>
          </div>
          
          <div v-if="userPermissions && typeof userPermissions === 'object' && Object.keys(userPermissions).length > 0" class="space-y-3">
            <div v-for="(permission, key) in userPermissions" :key="key" class="flex items-center gap-3">
              <div class="w-8 h-8 bg-gradient-to-br from-green-100 to-emerald-100 rounded-full flex items-center justify-center">
                <i class="fas fa-check text-green-600 text-sm"></i>
              </div>
              <div>
                <p class="text-sm font-medium text-gray-900">{{ formatPermissionKey(key) }}</p>
                <p class="text-xs text-gray-500">{{ permission ? 'Allowed' : 'Denied' }}</p>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-8">
            ...
            <p class="text-gray-500 text-sm">No specific permissions configured</p>
            <p class="text-xs text-gray-400 mt-2">Permissions data: {{ JSON.stringify(userPermissions) }}</p>
          </div>
        </div>
      </div>
    </div>
